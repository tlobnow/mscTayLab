...
SE <- read.csv("~/Documents/Github/mscTayLab/csv/sbatch_EVAL.csv")
SE <- SE %>% mutate(inv.RANK = 6-RANK,
                    can_form_filaments = case_when(FILE %in% c("MyD88_SPL2_short", "MyD88_SPL1_FL", "MYD88-DD_slim3", "ASC-PYD") ~ T,
                                                   !FILE %in% c("MyD88_SPL2_short", "MyD88_SPL1_FL", "MYD88-DD_slim3", "ASC-PYD") ~ NA))
...
#isolate <- SE %>% filter(FILE %in% c("CRP_MOUSE", "TNFa_MOUSE"))
...
ggplot() +
  geom_encircle(data = SE, aes(pLDDT, piTM, group=VIS, fill=VIS, col = VIS, alpha = 0.1), spread = 0.1) +
  geom_point(data = SE, aes(pLDDT, piTM, col = VIS, size = inv.RANK, alpha = 0.2)) +
  ggtitle(paste0("x = pLDDT, y = piTM, col = VIS, size = RANK")) #+
#  facet_wrap(~can_form_filaments) #+  geom_label(data = isolate, aes(pLDDT, piTM), label = isolate$FILE_MODEL)

ggplot() +
  geom_encircle(data = SE, aes(pLDDT, piTM, group=mainCat,fill=mainCat, col = mainCat, alpha = 0.1), spread = 0.1) +
  geom_point(data = SE, aes(pLDDT, piTM, col = mainCat, size = inv.RANK, alpha = 0.2)) +
  ggtitle(paste0("x = pLDDT, y = piTM, col = mainCat, size = RANK")) # +
  #facet_wrap(~can_form_filaments) + geom_label(data = isolate, aes(pLDDT, piTM, col = mainCat), label = isolate$FILE_MODEL)
...
SE.pca <- prcomp(SE[,c(9:14, 20)], center = TRUE,scale. = TRUE)
